...
class Game():
    ...
    def gameLoop(self, player, menu, level):
        ...
        while not done:
            ...
            ## osy
            # 자율주행 토글
            if pressed[pygame.K_1] and not player.auto_mode:
                player.auto_mode = True
                player.auto_mode_just_activated = True  # ← 새 경로 생성 트리거

            # 수동 조작 키가 눌렸으면 자율모드 해제
            manual_keys = [
                pygame.K_w, pygame.K_s, pygame.K_a, pygame.K_d,
                pygame.K_SPACE, pygame.K_h
            ]
            if any(pressed[key] for key in manual_keys):
                if player.auto_mode:
                    player.auto_mode = False
                    player.pure_pursuit = None  # 이전 경로 삭제
                    player.path_to_draw = []   # 생성 경로 시각화도 삭제
                    player.path_history = []   # 추종 경로 시각화도 삭제
                    if hasattr(player.tracker, '_target_idx'):
                        delattr(player.tracker, '_target_idx')

            if player.auto_mode:
                player.auto_drive(dt)
            else:
            ...
            # 주차 구역 ## osy
            if hasattr(map, "goal_rect") and map.goal_rect:
                pygame.draw.rect(self.surface, (0, 0, 255), map.goal_rect, 3)

            # 경로 시각화 ## osy
            if hasattr(player, "path_to_draw"):
                for (px, py) in player.path_to_draw:
                    pygame.draw.circle(self.surface, (0,255,0), (int(px), int(py)), 2)
            
            if hasattr(player, "path_history"):
                for (px, py) in player.path_history:
                    pygame.draw.circle(self.surface, (255,0,0), (int(px), int(py)), 2)
            
            # target_idx 시각화 (파란색 점)
            if hasattr(player.tracker, '_target_idx'):
                tx, ty = player.path_to_draw[player.tracker._target_idx]
                pygame.draw.circle(self.surface, (0, 0, 255), (int(tx), int(ty)), 5)
