...
class Map():
    ...
    def __init__(self, mapLayout):
        ...
        self.grid = [[int(cell) for cell in row] for row in self.tilesLayout]  ## osy
    ...
    def find_goal_area(self): ## osy
        """CSV에서 goal(1)로 구성된 사각형 영역을 찾아 Rect로 저장"""
        top, left = None, None
    ...
    def createTiles(self):
        ...
        for row in self.tilesLayout:
            x = 0
            for tile in row:
                if tile == str(1): # CSV 값이 1인 위치가 goal이고, 그 위치에 대해 self.goal이라는 Tile 객체를 생성한다는 것임.
